...
public class MyProtocol {
    ...
    private void sendMessage() {
        ...
        if (username.equalsIgnoreCase("all")) {
            ...
            try {
                ...
                if (read < 1024) {
                    List<RoutingInfo> tempRoutingTable = new ArrayList<>(routingTable);
                    for (RoutingInfo r : tempRoutingTable) {
                        if (r.address != SRC) {
                            sendPackets(read, temp, r.address, r.nextHopAddress, SRC);
                        }
                    }
                } else {
    ...
    private void receivePackets(ByteBuffer packet) throws InterruptedException {
        ...
        if (packet.get(1) == SRC || packet.get(3) == SRC) {
            ...
            if (packet.get(2) != 0) {
                //Check if the packet is destined to be broadcast
                if (packet.get(1) == (byte) 255) {
                    incomingBuffer.add(new IncomingInfo(getUsername( packet.get(0)),
                                                        ...
                                                        true));
                    for (RoutingInfo r : routingTable) {
                        if (r.address != SRC && r.address != packet.get(0)) {
                            sendPackets(packet.capacity(), packet, r.address, SRC, packet.get(0));
                        }
                    }
                } else {
                    ...
                    if (!packetSet.contains(packet)) {
                        ...
                        if (packet.get(4) == 2 || packet.get(4) == 10) {
                            if (q != -1) {
                                ...
                                incomingBuffer.get(
                                        incomingBuffer.size() - 1).message = buildMessage(
                                        incomingBuffer.get(incomingBuffer.size() - 1).message,
                                        packet);
                                incomingBuffer.get(
                                        incomingBuffer.size() - 1).timeRecieved = getCurrentTime();
